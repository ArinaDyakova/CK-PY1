...
def createString(list, delimiter, new_line):
    resultStr = ""
    for i in list:
        resultStr = resultStr + i + delimiter
    return resultStr[0:len(resultStr) - 1] + new_line


# # TODO реализовать функцию to_csv_file
def to_csv_file(filename, headers, rows, delimiter = ',', new_line = '\n'):
    with open(filename, 'w', encoding='utf8') as f:
        list = [createString(headers, delimiter, new_line)]
        for str in rows:
            list.append(createString(str, delimiter, new_line))
        f.writelines(list)
        f.close()
